...
#variables <- c('023c68873b', '06888ceac9', '361f93f4d1', '384bec5dd1', '7cb7913148', '8d0606b150', '91145d159d', '9a575e82a4', 'b835dfe10f', 'e16e640635', 'f1f0984934', 'id')
...
```{r target_interval}
# Cria a variável faixa de target
df1 <- df_train
...
# Criação da tabela cruzada entre a variável de faixa de target e a variável '06888ceac9'
CrossTable(df1$fxcnt_cat, df1$`06888ceac9`, prop.r = FALSE, prop.c = FALSE, prop.t = FALSE, prop.chisq = FALSE, chisq = TRUE)
```


```{r grafico_de_barras_empilhado}
# criando um vetor com os nomes das variáveis qualitativas
...
# Gráfico scatter plot entre a variável '253eb5ef11' e target
...
```{r modelo_stepwise_2 }
# Ajuste do modelo de regressão linear com seleção de variáveis stepwise (segunda tentativa)
...
# Excluir os outliers (segunda tentativa)
TrainSet_2 <- filter(TrainSet,TrainSet$rp>=-2&TrainSet$rp<=2) 
...
```{r modelo_stepwise_3 }
# Ajuste do modelo de regressão linear com seleção de variáveis stepwise (terceira tentativa)
modelo3 <- fit_model(TrainSet_2)
summary(modelo3)
```

```{r previsao_2}
# Adicionando previsões e resíduos ao conjunto de treinamento (terceira tentativa)
TrainSet_2 <- add_predictions_residues(TrainSet_2, modelo3)
```


```{r excluir_outlier_3}
# Excluir os outliers (terceira tentativa)
TrainSet_3 <- filter(TrainSet,TrainSet$rp>=-2&TrainSet$rp<=2)
...
```{r modelo_stepwise_4 }
# Ajuste do modelo de regressão linear com seleção de variáveis stepwise (quarta tentativa)
modelo4 <- fit_model(TrainSet_3)
summary(modelo4)
```

```{r previsao_3}
# Adicionando previsões e resíduos ao conjunto de treinamento (quarta tentativa)
TrainSet_3 <- add_predictions_residues(TrainSet_3, modelo4)
```

```{r excluir_outlier_4}
# Excluir os outliers (quarta tentativa)
TrainSet_4 <- filter(TrainSet,TrainSet$rp>=-2&TrainSet$rp<=2) 
...
```{r modelo_stepwise_5 }
# Ajuste do modelo de regressão linear com seleção de variáveis stepwise (quinta tentativa)
modelo5 <- fit_model(TrainSet_4)
```

```{r previsao_4}
# Adicionando previsões e resíduos ao conjunto de treinamento (quinta tentativa)
TrainSet_4 <- add_predictions_residues(TrainSet_4, modelo5)
```

```{r residuos}
# Plotando os resíduos em função das previsões do modelo. 
...
```{r excluir_outlier_5}
# Excluir os outliers (quinta tentativa)
...
```{r modelo_stepwise_6 }
# Ajuste do modelo de regressão linear com seleção de variáveis stepwise (sexta tentativa)
...
# Estes gráficos de dispersão mostram a relação entre diferentes variáveis e o alvo
